...
一款可在浏览器中游玩的文字冒险游戏。主篇《诺斯塔之塔》围绕过去与现在展开解谜，附篇《迷雾森林》以掷骰检定推动冒险。

游戏以 Twee 文本编写，可构建为单文件离线版，也可构建为图片、字体按需加载的网页版。源码适合用 Git 管理，编译后的 HTML 也可导入 Twine 2 编辑。
...
- **d20 检定**：`<<check "察觉" 10 "adv">>` / 豁免 `<<save "con" 15>>`，
  支持优势/劣势（双骰取高/低）与自然 20 必成、自然 1 必败（本游戏的检定约定）
